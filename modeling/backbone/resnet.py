...
class ResNet(nn.Module):
    def __init__(self, block, layers, output_stride):
        ...
        self.inplanes = 64
        if output_stride == 16:#在conv1和maxpool1已经缩小了4倍；所以这里只有2个或1个stride = 2.
            strides = [1, 2, 2, 1]
            dilations = [1, 1, 1, 2]
            blocks = [1, 2, 4]
        ...
        self.relu = nn.ReLU(inplace=True)
        self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)#输入尺寸是513*513，故此处选择ksize = 3
    ...
    def _make_MG_unit(self, block, planes, blocks = [1, 2, 4], stride = 1, dilation = 1, downsample = None):
